...
// Sizing constants. Tune in Story 7 polish if the cell feels too cramped.
const CELL_W = 14
const CELL_H = 52
const SEG_X = 2
const SEG_W = 10
...
const segmentH = computed(() =>
  (CELL_H - (segmentCount.value - 1) * SEG_GAP) / segmentCount.value,
)
...
// segment's rect, so stacking is `CELL_H - seg * segH - (seg - 1) * gap`.
function segmentY(seg: number): number {
  return CELL_H - seg * segmentH.value - (seg - 1) * SEG_GAP
}
...
function ariaLabel(slot: TodayDaySlot): string {
  ...
  return `${slot.label}: ${value} / ${props.scaleMax}`
}
</script>
