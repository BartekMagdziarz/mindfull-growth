<template>
  ...
  >
    ...
    <div class="flex items-center gap-2">
      ...
      >
        <AppIcon name="mood" class="text-lg text-on-surface" />
        <span class="flex-1 truncate text-left font-semibold text-on-surface">{{
          t('planning.calendar.wellness.emotions')
        }}</span>
      </button>
      ...
      >
        ...
        <div v-if="!expanded" class="flex items-end gap-1">
          ...
          >
            ...
            >
              <div
                v-if="dayEmotionData.has(slot.dateKey)"
                class="w-full rounded-t-sm"
                :style="{
                  height: emotionBarHeight(dayEmotionData.get(slot.dateKey)!.logCount) + '%',
                  background: emotionGradient(
                    dayEmotionData.get(slot.dateKey)!.quadrantProportions,
                  ),
                }"
              />
            </div>
            ...
            <template v-for="week in weekGrid" :key="week.slots[0].dateKey">
              ...
              >
                <div
                  ...
                  :style="{
                    height:
                      emotionBarHeight(dayEmotionData.get(slot.dateKey)!.logCount) + '%',
                    background: emotionGradient(
                      dayEmotionData.get(slot.dateKey)!.quadrantProportions,
                    ),
                  }"
...
import type { CalendarDaySlot, DayEmotionSummary } from '@/utils/wellnessCalendar'
...
const maxLogCount = computed(() => {
  ...
  for (const key of visibleDateKeys.value) {
    const d = props.dayEmotionData.get(key)
    if (d && d.logCount > max) max = d.logCount
  }
  return max
})

function emotionBarHeight(logCount: number): number {
  if (maxLogCount.value <= 0) return 0
  return Math.max(8, (logCount / maxLogCount.value) * 100)
}

const GRADIENT_SEGMENTS: { quadrant: Quadrant; css: string }[] = [
  {
    quadrant: 'high-energy-high-pleasantness',
    css: 'var(--color-quadrant-high-energy-high-pleasantness-selected)',
  },
  {
    quadrant: 'high-energy-low-pleasantness',
    css: 'var(--color-quadrant-high-energy-low-pleasantness-selected)',
  },
  {
    quadrant: 'low-energy-low-pleasantness',
    css: 'var(--color-quadrant-low-energy-low-pleasantness-selected)',
  },
  {
    quadrant: 'low-energy-high-pleasantness',
    css: 'var(--color-quadrant-low-energy-high-pleasantness-selected)',
  },
]

function emotionGradient(proportions: Record<Quadrant, number>): string {
  const active = GRADIENT_SEGMENTS.filter((s) => proportions[s.quadrant] > 0)
  if (active.length === 0) return 'transparent'
  if (active.length === 1) return active[0].css

  let pct = 0
  const stops = active.map((s) => {
    const start = pct
    pct += proportions[s.quadrant] * 100
    return `${s.css} ${start}% ${pct}%`
  })
