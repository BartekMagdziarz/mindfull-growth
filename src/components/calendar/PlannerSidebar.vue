                  ...
                  <section v-if="section.needsPlanning.length > 0 || section.planned.length > 0" class="space-y-2">
                    ...
                    <div v-if="section.needsPlanning.length > 0" class="space-y-2">
                      ...
                      <PlannerMeasurementRowCard
                        ...
                        :pick-weeks-label="t('planning.calendar.planner.pickWeeks')"
...
function wholeLabel(row: PlannerDisplayRow): string {
  return row.cadence === 'monthly'
    ? t('planning.calendar.planner.wholeMonth')
    : t('planning.calendar.planner.allWeeks')
}

function goalDisplaySections(goal: GoalSection): PlannerSectionRows[] {
  return buildMonthSections(goal.keyResults)
}

function buildMonthSections(rows: PlannerMeasurementRow[]): PlannerSectionRows[] {
  return [
    makeSection('monthly', t('planning.calendar.planner.groups.monthly'), rows.filter(row => row.cadence === 'monthly')),
    makeSection('weekly', t('planning.calendar.planner.groups.weekly'), rows.filter(row => row.cadence === 'weekly')),
...
function buildDisplayRow(row: PlannerMeasurementRow): PlannerDisplayRow {
  ...
  if (row.scheduledDayRefs.length > 0) {
    placementSummary = t('planning.calendar.planner.daysCompact', { n: row.scheduledDayRefs.length })
  } else if (wholePeriod) {
    placementSummary = row.cadence === 'monthly'
      ? t('planning.calendar.planner.wholeMonth')
      : t('planning.calendar.planner.allWeeks')
  } else if (plannedWeekCount > 0) {
