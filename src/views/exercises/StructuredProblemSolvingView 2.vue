<template>
  <div class="mx-auto w-full max-w-3xl px-4 py-6 pb-24">
    <div class="flex items-center gap-4 mb-6">
      ...
      <div>
        <h1 class="text-xl font-bold text-on-surface">{{ t('exercises.cards.structuredProblemSolving.title') }}</h1>
        <p class="text-sm text-on-surface-variant">{{ t('exercises.cards.structuredProblemSolving.subtitle') }}</p>
      </div>
    </div>
    ...
    <div v-if="spsStore.sortedSessions.length > 0" class="mt-8">
      <h2 class="text-lg font-semibold text-on-surface mb-3">{{ t('exercises.views.pastSessions') }}</h2>
      <div class="space-y-3">
        <AppCard
          v-for="session in spsStore.sortedSessions"
          :key="session.id"
          padding="md"
        >
          <div class="flex justify-between items-start">
            <div>
              ...
              <p class="text-xs text-on-surface-variant mt-1">
                ...
                &middot; {{ session.solutions.length }} solutions
