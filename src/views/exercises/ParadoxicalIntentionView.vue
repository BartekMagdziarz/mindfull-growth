<template>
  <div class="mx-auto w-full max-w-3xl px-4 py-6 pb-24">
    <div class="flex items-center gap-4 mb-6">
      ...
      <div>
        <h1 class="text-xl font-bold text-on-surface">{{ t('exercises.cards.paradoxicalIntention.title') }}</h1>
        <p class="text-sm text-on-surface-variant">{{ t('exercises.cards.paradoxicalIntention.subtitle') }}</p>
      </div>
    </div>
    ...
    <div v-if="paradoxicalStore.sortedLabs.length > 0" class="mt-8">
      <h2 class="text-lg font-semibold text-on-surface mb-3">{{ t('exercises.views.pastLabs') }}</h2>
      <div class="space-y-3">
        <AppCard
