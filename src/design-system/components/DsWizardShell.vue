<template>
  <section class="mg-v2-wizard-shell">
    <aside v-if="steps.length" class="mg-v2-wizard-shell__chapters" aria-label="Rozdziały">
      <header><span>Ścieżka</span><small>{{ current + 1 }}/{{ steps.length }}</small></header>
      <ol>
        <li v-for="(step, index) in steps" :key="step.id" :class="{ active: index === current, done: index < current }">
    ...
    <div class="mg-v2-wizard-shell__stage">
      <header class="mg-v2-wizard-shell__header">
        <div><span v-if="eyebrow">{{ eyebrow }}</span><h1>{{ title }}</h1><p v-if="description">{{ description }}</p></div>
        <slot name="header-actions" />
      </header>
      <div class="mg-v2-wizard-shell__body"><slot /></div>
      <footer v-if="$slots.footer" class="mg-v2-wizard-shell__footer"><slot name="footer" /></footer>
    </div>
  </section>
</template>
