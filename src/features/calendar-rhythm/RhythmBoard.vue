        ...
        <div v-else-if="row.kind === 'series'" class="rb-row rb-series" :class="[`rb-series--${row.series.markKind}`, { observation: row.series.evidenceRole === 'observation', 'rb-series--plot': usesLine(row.series) }]" role="row">
          ...
          <div class="rb-series__cells" :style="{ '--cols': units.length }">
            <button v-for="point in row.series.points" :key="point.unitRef" type="button" class="rb-cell rb-cell--series" :class="{ future: point.state === 'future' }" :title="usesLine(row.series) ? undefined : `${row.series.label}, ${unitLabel(point.unitRef)}: ${point.readout}`" :aria-label="`${row.series.label}, ${unitLabel(point.unitRef)}: ${point.readout} · ${openTitle(unitOf(point.unitRef))}`" @click="emit('open-unit', point.unitRef)">
              ...
              <RhythmSeriesMark v-else :series="row.series" :point="point" :scale-max="row.scaleMax" />
            </button>
            <template v-if="usesLine(row.series)">
              <RhythmSeriesLine :series="row.series" :grain="grainOf(row.series)" :unit-labels="units.map(u => unitLabel(u.ref))" @select="ref => emit('open-unit', ref)" />
        ...
        <div v-else-if="row.kind === 'entries-kind'" class="rb-row rb-series rb-series--entries" role="row">
          ...
          <div class="rb-series__cells" :style="{ '--cols': units.length }">
            <button v-for="e in row.cells" :key="e.unitRef" type="button" class="rb-cell rb-cell--series" :title="openTitle(unitOf(e.unitRef))" :aria-label="`${row.label}, ${unitLabel(e.unitRef)}: ${e.kinds[row.kindId].count} wpisów w ${e.kinds[row.kindId].days} dniach · ${openTitle(unitOf(e.unitRef))}`" @click="emit('open-unit', e.unitRef)">
              <RhythmEntrySlots :data="e.kinds[row.kindId]" :kind-label="row.label" />
            </button>
          </div>
          <span v-if="hasSigma" class="rb-sigma" />
        </div>
...
import RhythmSeriesLine from './RhythmSeriesLine.vue'
import RhythmRatingsMini from './RhythmRatingsMini.vue'
import RhythmSeriesMark from './RhythmSeriesMark.vue'
import { isLineKind, rangeLabel, periodTitle, seriesAxis, supportsFineGrain, type Grain, type Scale, type SeriesPoint, type SeriesProjection, type TimeUnit } from './rhythmProjections'
import type { BoardRow, ViewOption } from './rhythmRows'
import type { RhythmObject } from './rhythmScenario'

const props = defineProps<{ rows: BoardRow[]; units: TimeUnit[]; scale: Scale; objects: RhythmObject[]; /** Serie w drobnej granulacji (klucze obiektów). */ fine: string[]; view: string; options: ViewOption[] }>()
...
.rb-view:has(.rb-view__select:focus-visible) { outline: 2px solid var(--cp-mark); outline-offset: 3px; }
.rb-view__chev { flex: none; font-size: 18px; color: var(--mg-color-muted); }
.rb-axis__track { position: absolute; top: 0; bottom: 0; left: calc(var(--pad) + var(--name) - 2px); right: calc(var(--pad) - 4px); border-radius: 18px 15px 19px 16px; background: var(--cp-card); box-shadow: var(--mg-shadow-inset-sm); }
.rb-axis__unit, .rb-sigma--head { position: relative; z-index: 1; }
.rb-axis__unit { display: grid; place-items: center; min-height: 36px; margin: 4px 0; padding: 3px 4px; border: 1px solid transparent; border-radius: 13px 16px 11px 17px; background: transparent; color: var(--mg-color-muted); font-size: 12.5px; font-weight: 800; cursor: pointer; }
...
.rb-sigma--head { color: var(--mg-color-muted); font-size: 11px; font-weight: 900; letter-spacing: .06em; }
.rb-sigma__well { padding: 4px 9px; border-radius: 11px 14px 10px 13px; background: var(--cp-field); white-space: nowrap; }
.rb-sigma__well--done { color: var(--mg-color-ink); }
.rb-sigma__well--short, .rb-sigma__well--unrecorded, .rb-sigma__well--planned { color: var(--mg-color-muted); }

/* kartka z wierszami */
.rb-cloud { padding: 8px 0 8px; border: 1px solid var(--cp-line); border-radius: 25px 30px 24px 28px; background: var(--cp-card); box-shadow: var(--mg-shadow-rhythm-card); }
.rb-name { display: flex; align-items: center; gap: 10px; min-width: 0; padding: 8px 8px; border: 0; border-radius: 13px 17px 12px 16px; background: transparent; color: var(--mg-color-ink); font: inherit; text-align: left; }
.rb-name--series { padding-left: 14px; font-size: 13.5px; font-weight: 700; color: var(--mg-color-muted); }
.rb-name--series small { color: var(--mg-color-muted); font-weight: 600; }
.rb-name--sub { padding: 6px 8px 0 14px; color: var(--mg-color-muted); font-size: 10px; font-weight: 900; letter-spacing: .12em; text-transform: uppercase; }
.rb-name__text { overflow: hidden; text-overflow: ellipsis; }
.rb-name__icon { display: inline-grid; place-items: center; flex: none; width: 31px; height: 30px; border-radius: 52% 48% 54% 46% / 47% 53% 46% 54%; color: var(--cp-mark); background: var(--cp-inner); font-size: 19px; }
.rb-name__icon--small { width: 25px; height: 24px; font-size: 15px; }

.rb-cell { display: grid; place-items: center; min-height: 46px; border: 0; border-radius: 12px 16px 10px 14px; background: transparent; color: var(--mg-color-ink); cursor: pointer; }
...
.rb-focus-band { display: flex; align-items: center; justify-content: center; width: calc(100% - 18px); max-width: 170px; height: 22px; border: 1px solid var(--cp-edge-strong); border-radius: 9px 13px 8px 11px; background: var(--cp-inner); box-shadow: var(--mg-shadow-raised-sm); }
.rb-focus-band .material-symbols-outlined { font-size: 14px; }
.rb-focus-gap { width: calc(100% - 18px); max-width: 170px; height: 1px; background: var(--cp-hairline); }

/* serie: wiersze ~20% wyższe niż komórki fokusu, żeby wykresy miały pion */
.rb-series { min-height: 70px; }
.rb-series + .rb-series, .rb-series + .rb-sublabel { border-top: 1px dashed var(--cp-line); }
.rb-series.observation .rb-name--series { color: var(--mg-color-muted); }
...
.rb-axis-bounds { visibility: hidden; position: absolute; right: 100%; top: 8px; bottom: 8px; display: flex; flex-direction: column; justify-content: space-between; margin-right: 8px; color: var(--mg-color-muted); font-size: 10px; font-weight: 700; font-variant-numeric: tabular-nums; text-align: right; pointer-events: none; }
...
.rb-note { min-height: 44px; color: var(--mg-color-muted); font-size: 13.5px; }
