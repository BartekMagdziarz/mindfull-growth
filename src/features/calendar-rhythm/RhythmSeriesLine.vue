...
import { isLineKind, seriesAxis, type Grain, type SeriesProjection } from './rhythmProjections'
...
.sl-sep { stroke: var(--cp-line); stroke-width: 1; stroke-dasharray: 2 4; }
.sl-base { stroke: color-mix(in srgb, var(--cp-mark) 35%, transparent); stroke-width: 1.2; }
.sl-target { stroke: var(--cp-accent-strong); stroke-width: 1.25; stroke-dasharray: 3 4; }
.sl-line { stroke: var(--cp-mark); stroke-width: 2; }
.sl-echo { stroke: var(--cp-edge); stroke-width: 3.2; }
.sl-bridge { stroke: var(--cp-edge-strong); stroke-width: 1.2; stroke-dasharray: 2 5; }
.sl-ring { stroke: var(--cp-page); stroke-width: var(--ring, 12px); }
.sl-dot { stroke: var(--mg-color-ink); stroke-width: var(--dot, 7.5px); pointer-events: visibleStroke; cursor: pointer; }
.sl-bar { stroke: var(--cp-accent); stroke-width: var(--bar, 7px); pointer-events: visibleStroke; cursor: pointer; }
.sl-bar-echo { stroke: var(--cp-mark-soft); stroke-width: var(--bar, 7px); }
.sl-bar:hover, .sl-dot:hover { stroke: var(--mg-color-ink); }
.sl--observation .sl-line { stroke: var(--cp-accent-strong); }
.sl--observation .sl-dot { stroke: var(--cp-mark); }
