/**
 * Wiersze planszy jako czysta funkcja stanu UI: tabela pokazuje jedno „spojrzenie” naraz
...
import type { PriorityScenario, ScenarioObject, ScenarioPriority } from '~lab/lab/calendarPriorityScenario'
...
const FAMILY_ICON: Record<ScenarioObject['family'], string> = { goal: 'flag', keyResult: 'flag', habit: 'repeat', tracker: 'monitoring', intention: 'lightbulb' }
...
export type BoardRow =
  | { kind: 'focus-lane'; id: string; priority: ScenarioPriority; focused: boolean[] }
...
export function viewOptions(s: PriorityScenario, scale: Scale, ref: string, units: TimeUnit[]): ViewOption[] {
  const { focus, rest } = directionsForPeriod(s, scale, ref, units)
  const withActivity = (p: ScenarioPriority) => activeInPeriod(s, objectsForPriority(s, p.key), units).length > 0
...
export function resolveView(s: PriorityScenario, scale: Scale, ref: string, units: TimeUnit[], requested: string | null): string {
...
export function viewLabel(s: PriorityScenario, view: string): string {
  if (view.startsWith('dir:')) return s.priorities.find(p => p.key === view.slice(4))?.title ?? 'Kierunek'
  if (view.startsWith('type:')) return FAMILY_LABEL[view.slice(5) as ScenarioObject['family']] ?? 'Obiekty'
  return view === 'focus' ? 'Fokus' : view === 'reflection' ? 'Oceny' : 'Wpisy'
}

function seriesRows(s: PriorityScenario, objects: ScenarioObject[], units: TimeUnit[], scale: Scale, ref: string, parentId: string, ui: RowsUi): BoardRow[] {
...
export function buildRows(s: PriorityScenario, scale: Scale, ref: string, units: TimeUnit[], view: string, ui: RowsUi): BoardRow[] {
