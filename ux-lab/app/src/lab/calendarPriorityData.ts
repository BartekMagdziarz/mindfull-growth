/**
 * Czyste projekcje dla „05 · Rytm kierunków”: jednostki czasu z dwoma zakresami
 * (pełny / przecięcie z okresem), obecność, serie per entryMode, fokus, refleksja, wpisy,
 * treść panelu. Bez Vue, bez runtime produktu — tylko czyste helpery okresów.
 *
 * Reguły z planu §05/§05b:
 *  - znaczniki i sumy datowanych wpisów korzystają z PRZECIĘCIA z oglądanym okresem,
 *  - brak zapisu ≠ niewykonanie; undefined/null = brak, 0 = zapis zera,
 *  - rodzaj znacznika serii wynika z entryMode + celu + kadencji (słownik resolverów produktu),
 *  - cel efektywny = bazowy → override miesiąca → override tygodnia; w częściowym tygodniu brak celu,
 *  - Σ tylko tam, gdzie wartość okresu jest agregatem komórek.
 */
...
export const COMPASS = ['Balans', 'Sens', 'Rozwój', 'Zasady', 'Wpływ'] as const
export const FAMILY_LABEL: Record<ScenarioObject['family'], string> = {
  goal: 'Cele', keyResult: 'Cele', habit: 'Nawyki', tracker: 'Trackery', intention: 'Intencje tygodnia',
}
export const TYPE_ORDER: ScenarioObject['family'][] = ['keyResult', 'habit', 'tracker', 'intention']
...
export function objectByKey(s: PriorityScenario, key: string): ScenarioObject | undefined {
  return s.objects.find(o => o.key === key)
}
export function priorityKeysOf(s: PriorityScenario, object: ScenarioObject): string[] {
...
export function measurableObjects(s: PriorityScenario): ScenarioObject[] {
  return s.objects.filter(o => o.family !== 'goal')
}
export function objectsForPriority(s: PriorityScenario, priorityKey: string): ScenarioObject[] {
  return measurableObjects(s).filter(o => priorityKeysOf(s, o).includes(priorityKey))
}
export function objectsWithoutPriority(s: PriorityScenario): ScenarioObject[] {
  return measurableObjects(s).filter(o => priorityKeysOf(s, o).length === 0)
}
export function goalTitle(s: PriorityScenario, object: ScenarioObject): string | undefined {
...
export function entryInUnit(entry: ScenarioEntry, unit: TimeUnit): boolean {
...
export function assignmentInUnit(a: ScenarioAssignment, unit: TimeUnit): boolean {
...
export function assignmentPlansUnit(a: ScenarioAssignment, unit: TimeUnit): boolean {
...
export function isRecorded(entry: ScenarioEntry): boolean {
...
export function presenceCell(s: PriorityScenario, objects: ScenarioObject[], unit: TimeUnit): CellProjection {
...
export interface SeriesProjection {
  ...
  sublabel?: string
  unit?: string
  entryMode: ScenarioObject['entryMode']
  cadence: ScenarioObject['cadence']
  aggregation: 'sum' | 'average' | 'last' | 'count'
  operator?: 'min' | 'max' | 'gte' | 'lte'
  ratingScale?: { min: number; max: number }
  markKind: MarkKind
  evidenceRole: ScenarioObject['evidenceRole']
...
export function resolveMarkKind(object: ScenarioObject, scale: Scale): MarkKind {
...
function aggregationOf(object: ScenarioObject): SeriesProjection['aggregation'] {
  const t = object.target
...
export function effectiveTarget(s: PriorityScenario, object: ScenarioObject, monthRef: MonthRef, weekRef?: WeekRef): Target | undefined {
...
function multiThreshold(object: ScenarioObject): number {
...
function multiPoints(object: ScenarioObject, entry: ScenarioEntry): number {
...
export function multiDayMet(object: ScenarioObject, entry: ScenarioEntry): boolean {
...
function aggregate(object: ScenarioObject, entries: ScenarioEntry[]): number | null {
...
function daySlotsFor(object: ScenarioObject, unit: TimeUnit, entries: ScenarioEntry[], clock: DayRef): DaySlot[] {
...
function targetValueFor(object: ScenarioObject, s: PriorityScenario, unit: TimeUnit, scale: Scale): number | null {
...
function readoutFor(object: ScenarioObject, value: number | null, target: number | null, point: { partial: boolean; planned: boolean; entryDays: number; state: TimeState }, operator?: string): string {
...
export function seriesFor(s: PriorityScenario, object: ScenarioObject, units: TimeUnit[], scale: Scale, focusRef: string): SeriesProjection {
...
export function activeInPeriod(s: PriorityScenario, objects: ScenarioObject[], units: TimeUnit[]): ScenarioObject[] {
...
export function focusForUnit(s: PriorityScenario, unit: TimeUnit): FocusProjection {
  if (unit.kind === 'month') {
...
export function directionsForPeriod(s: PriorityScenario, scale: Scale, ref: string, units: TimeUnit[]): { focus: ScenarioPriority[]; rest: ScenarioPriority[] } {
...
export interface ReflectionProjection {
  ...
  weekly?: WeeklyReflectionRecord
  monthly?: MonthlyReflectionRecord
}

export function reflectionForUnit(s: PriorityScenario, unit: TimeUnit): ReflectionProjection {
  if (unit.kind === 'week') {
...
export function ownReflection(s: PriorityScenario, scale: Scale, ref: string): ReflectionProjection | null {
  if (scale === 'month') {
...
export function entriesForUnit(s: PriorityScenario, unit: TimeUnit): EntriesProjection {
...
function valueLabelFor(object: ScenarioObject, entry: ScenarioEntry): string | undefined {
...
export function activitiesFor(s: PriorityScenario, objects: ScenarioObject[], unit: TimeUnit): { dated: PanelActivity[]; scoped: PanelActivity[]; observations: PanelActivity[] } {
...
export interface FocusDetail {
  ...
  objects: { object: ScenarioObject; priorityKeys: string[] }[]
  /** Obiekty fokusu tygodnia bez związku z priorytetami miesiąca (dryf, cicha lista). */
  drift: ScenarioObject[]
  source: FocusProjection['source']
}

export function focusDetail(s: PriorityScenario, unit: TimeUnit): FocusDetail {
  ...
  if (unit.kind === 'week') {
    ...
    const objects = (plan?.topObjectKeys ?? []).map(k => objectByKey(s, k)).filter((o): o is ScenarioObject => !!o).map(object => ({ object, priorityKeys: priorityKeysOf(s, object) }))
...
export function journalIn(s: PriorityScenario, unit: TimeUnit): JournalRecord[] { return recordsIn(s.journal, unit) }
export function emotionsIn(s: PriorityScenario, unit: TimeUnit): EmotionRecord[] { return recordsIn(s.emotions, unit) }
export function exercisesIn(s: PriorityScenario, unit: TimeUnit): ExerciseRecord[] { return recordsIn(s.exercises, unit) }
...
function familyGroupOf(object: ScenarioObject): FamilyGroup | null {
 ...
 */
export function objectCompletion(s: PriorityScenario, object: ScenarioObject, scale: Scale, ref: string, units: TimeUnit[]): { ratio: Ratio; basis: CompletionBasis } {
...
export function periodStats(s: PriorityScenario, scale: Scale, ref: string, units: TimeUnit[]): PeriodStats {
...
export function periodRating(s: PriorityScenario, scale: Scale, ref: string, units: TimeUnit[]): PeriodRating {
  if (scale === 'year') {
    const past = units.filter(u => u.state === 'past')
    const records = past.map(u => reflectionForUnit(s, u).monthly).filter((r): r is MonthlyReflectionRecord => !!r)
