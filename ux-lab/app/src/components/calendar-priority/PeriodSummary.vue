<template>
  <section class="ps" :aria-label="`Podsumowanie · ${title}`">
    ...
    <div v-if="open" class="ps__body">
      ...
      <ReflectionBody v-else-if="reflection?.exists" :reflection="reflection" :priority-title="priorityTitle" hide-ratings />
...
import ReflectionBody from './ReflectionBody.vue'
import { AREAS, COMPASS, FAMILY_GROUPS, directionsForPeriod, ownReflection, periodRating, periodStats, periodTitle, truncate, type CompletionBasis, type Scale, type TimeUnit } from '~lab/lab/calendarPriorityData'
import type { PriorityScenario } from '~lab/lab/calendarPriorityScenario'
import { FAMILY_VIEW } from '~lab/lab/calendarPriorityRows'
import type { PeriodRef } from '@product/domain/period'
import { getPeriodBounds } from '@product/utils/periods'

const props = defineProps<{
  scenario: PriorityScenario; scale: Scale; periodRef: string; units: TimeUnit[]; open: boolean
...
<style scoped>
.ps { display: grid; gap: 14px; padding: 16px 20px 16px; border: 1px solid var(--cp-line); border-radius: 25px 30px 24px 28px; background: var(--cp-card); box-shadow: var(--cp-shadow-card); }
.ps__eyebrow { color: rgb(var(--sky-700)); font-size: 11px; font-weight: 900; letter-spacing: .1em; text-transform: uppercase; }
.ps__eyebrow small { color: rgb(var(--neo-muted)); font-size: 11px; letter-spacing: 0; text-transform: none; }
...
.ps__chip { display: inline-flex; align-items: center; gap: 9px; max-width: 260px; padding: 3px 12px 3px 4px; border: 1px solid var(--cp-line); border-radius: 17px 14px 18px 15px; background: var(--cp-inner); color: rgb(var(--sky-800)); font: inherit; font-size: 13.5px; font-weight: 800; text-align: left; cursor: pointer; box-shadow: var(--cp-shadow-card); }
.ps__chip:hover { background: white; }
.ps__chip.is-active { background: white; }
.ps__chip:hover .ps__orb { display: grid; place-items: center; flex: none; width: 30px; height: 29px; border-radius: 52% 48% 54% 46% / 47% 53% 46% 54%; color: rgb(var(--sky-700)); background: var(--cp-inner); }
.ps__orb { display: grid; place-items: center; flex: none; width: 30px; height: 29px; border-radius: 52% 48% 54% 46% / 47% 53% 46% 54%; color: rgb(var(--sky-700)); background: var(--cp-inner); }
...
.ps__empty { font-size: 13px; color: rgb(var(--neo-muted)); margin: 0; }
...
.ps__drill { position: absolute; top: 10px; right: 10px; display: grid; place-items: center; width: 30px; height: 29px; border: 0; border-radius: 52% 48% 54% 46% / 47% 53% 46% 54%; background: transparent; color: rgb(var(--neo-muted)); cursor: pointer; }
.ps__drill:hover, .ps__drill[aria-pressed='true'] { background: var(--cp-inner); color: rgb(var(--sky-800)); }
...
.ps__col .e, .ps__legend .e { background: rgb(var(--rose-400)); }
.ps__col .s, .ps__legend .s { background: var(--cp-accent); }
.ps__col i.none { background: var(--cp-inner); min-height: 3px; }
.ps__col i b { position: absolute; left: 50%; bottom: 100%; transform: translate(-50%, -2px); color: rgb(var(--sky-800)); font-size: 10.5px; font-weight: 800; font-variant-numeric: tabular-nums; }
.ps__bars small { color: rgb(var(--neo-muted)); font-size: 10.5px; font-weight: 800; text-align: center; }
.ps__legend { display: flex; align-items: center; gap: 6px; color: rgb(var(--neo-muted)); font-size: 11px; font-weight: 800; }
.ps__legend i { display: inline-block; width: 8px; height: 8px; border-radius: 3px 2px 3px 2px; }
.ps__none { margin: 4px 0 0; color: rgb(var(--color-on-surface-variant)); font-size: 14px; }
...
.ps__tile--none .ps__tile-label { color: rgb(var(--neo-muted)); }
.ps__tile-label { grid-area: label; color: rgb(var(--sky-800)); font-size: 16px; font-weight: 800; letter-spacing: .01em; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.ps__pct { grid-area: pct; color: rgb(var(--sky-800)); font-size: 22px; font-weight: 900; letter-spacing: -.01em; font-variant-numeric: tabular-nums; }
.ps__pct span { margin-left: 1px; color: rgb(var(--neo-muted)); font-size: 12px; font-weight: 800; }
.ps__dash { color: rgb(var(--neo-muted)); }
.ps__blob { grid-area: blob; display: block; width: 44px; height: 42px; border-radius: 52% 48% 54% 46% / 47% 53% 46% 54%; background: linear-gradient(to top, var(--cp-accent) var(--fill), var(--cp-inner) var(--fill)); box-shadow: inset 0 0 0 1px rgb(var(--sky-400) / .35); transform: rotate(-4deg); }
.ps__tile:nth-child(even) .ps__blob { border-radius: 47% 53% 49% 51% / 54% 46% 53% 47%; transform: rotate(3deg); }
.ps__tile--none .ps__blob { background: var(--cp-inner); box-shadow: inset 0 0 0 1px rgb(var(--neo-border) / .3); }
.ps__quote { display: flex; gap: 10px; align-items: baseline; margin: 0; padding-left: 12px; border-left: 2px solid rgb(var(--sky-300)); color: rgb(var(--color-on-surface)); font-size: 14px; line-height: 1.5; }
.ps__quote span { flex: none; color: rgb(var(--neo-muted)); font-size: 12px; font-weight: 700; }
